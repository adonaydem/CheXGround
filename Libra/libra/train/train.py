...
def preprocess(
    ...
    if conversation_lib.default_conversation.sep_style in {conversation_lib.SeparatorStyle.LLAMA_2, conversation_lib.SeparatorStyle.MISTRAL}:
    ...
    if model_args.vision_tower is not None:
        if "mistral" in model_args.model_name_or_path.lower():
            ...
            if (
                ...
                training_args.fsdp_config["transformer_layer_cls_to_wrap"] = ["GemmaDecoderLayer"]

            model = LibraGemmaForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
                **bnb_model_from_pretrained_args
